...
# NOTE: Further refactor is possible to eliminate code duplication between
# DataStream and CryptoDataStream
...
class CryptoDataStream(_DataStream):
    def __init__(self,
        ...
        if exchanges:
            endpoint += f'?exchanges={",".join(exchanges)}'
    ...
    async def _run_forever(self):
        ...
        while not (self._trade_handlers or self._quote_handlers
            ...
            if not self._stop_stream_queue.empty():
                ...
                return
            await asyncio.sleep(0.1)
        log.info('started crypto data stream')
        retries = 0
        self._running = False
        while True:
            try:
                ...
                if not self._running:
                    log.info('terminating crypto data stream')
                    break
                await asyncio.sleep(0.01)
